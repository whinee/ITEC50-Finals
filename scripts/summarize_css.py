...
EXCLUDE = {
    ...
    "folders": [
        "src/static/stylesheets/downloaded",  # vendor CSS — don't document these
    ],
    # Specific files to always exclude
    "files": [
        # "src/static/stylesheets/legacy.css",
    ],
...
def resolve_files() -> list[str]:  # noqa: C901
    ...
    exclude_folder_paths = [Path(f).resolve() for f in EXCLUDE.get("folders", [])]
    ...
    for file in EXCLUDE.get("files", []):
        p = Path(file)
        # match by resolved path so relative vs absolute doesn't matter
...
def next_non_blank_selector(lines: list[str], after: int) -> Optional[str]:
    """
...
def parse_css(path: str) -> tuple[list[CSSNode], list[LintError]]:
    ...
    def find_or_create_child(parent_list: list[CSSNode], name: str, full_path: str, depth: int) -> CSSNode:
    ...
    def insert_node(parts: list[str], line: int, kind: str, line_no: int) -> CSSNode | None:
    ...
    for ev in events:
        if ev.kind == "start":
            node = insert_node(ev.parts, ev.line, "start", ev.line)
            if node:
            ...
            node = insert_node(ev.parts, ev.line, "end", ev.line)
            if node:
...
def render_ascii_tree(roots: list) -> str:
...
def generate_md(css_path: str, roots: list[CSSNode], lint_errors: list[LintError]) -> str:
    lines: list[str] = [
        f"# CSS Summary: `{Path(css_path).name}`",
        "",
...
def _forest_node(node, indent: int = 2) -> list[str]:
    """Recursively renders a CSSNode as forest package syntax."""
    pad   = " " * indent
    label = tex_escape(node.name)

    if not node.is_inline and node.line_start:
        loc = f"line {node.line_start}"
        if node.line_end:
            loc += f"--{node.line_end}"
        label += r" \textit{\scriptsize(" + tex_escape(loc) + r")}"

    visible = [c for c in node.children if not c.is_inline]
    ...
    for child in visible:
        lines.extend(_forest_node(child, indent + 2))
    lines.append(f"{pad}]")
    return lines

def render_forest_tree(roots: list) -> str:
    r"""Returns a full LaTeX forest environment string. Caller is responsible for \usepackage{forest} in the preamble."""
    if not roots:
        return r"\textit{(no sections)}"

    lines = [
        r"\begin{forest}",
        r"  for tree={",
        r"    font=\ttfamily\small,",
        r"    grow'=0,",
        r"    child anchor=west,",
        ...
        r"      (!u.south west) +(7.5pt,0) |- (.child anchor)\forestoption{edge label};",
        r"    },",
        r"    before typesetting nodes={",
        r"      if n=1{insert before={[,phantom]}}{}",
        r"    },",
        r"    fit=band,",
        r"    before computing xy={l=15pt},",
        r"  }",
        r"  [{\textbf{(root)}}",   # virtual root node
    ]

    for root in roots:
        lines.extend(_forest_node(root, indent=4))

    lines.append(r"  ]")
    lines.append(r"\end{forest}")
    return "\n".join(lines)
...
def generate_tex(css_path: str, roots: list[CSSNode], lint_errors: list[LintError]) -> str:
    filename_escaped = tex_escape(Path(css_path).name)
    ...
    lines: list[str] = [
        ...
        r"",
        r"\section*{" + f"CSS: {filename_escaped}" + r"}",
        r"\textit{Source: \texttt{" + path_escaped + r"}} \\",
        r"\textit{Generated: " + tex_escape(date_str) + r"} \\[0.5em]",
        r"",
    ]
    ...
    if lint_errors:
        ...
        for err in lint_errors:
            lines.append(r"  \item \textbf{Line " + str(err.line) + r":} " + tex_escape(err.message))
        lines.append(r"\end{itemize}")
    else:
        lines.append(r"\textit{No lint issues found.} \\")
    lines.append(r"")

    # tree — forest overview
    lines.append(r"\subsection*{Section Tree}")
    lines.append(r"")
    lines.append(r"\noindent")
    lines.append(render_forest_tree(roots))
    lines.append(r"")
...
def generate_md_index(css_files: list[str], md_dir: str) -> str:
    ...
    for css_path in css_files:
        stem     = Path(css_path).stem
        md_name  = f"{stem}.md"
        lines.append(f"- [{Path(css_path).name}](./{md_name})")
    lines.append("")
    return "\n".join(lines)
...
def generate_tex_index(css_files: list[str], tex_dir: str) -> str:
    date_str = datetime.now().strftime("%Y-%m-%d %H:%M")
    lines = [
        f"% CSS Documentation Index",
        f"% Generated: {date_str}",
        r"",
        r"\section*{CSS Documentation Index}",
        r"",
    ]
    for css_path in css_files:
        stem     = Path(css_path).stem
        tex_name = f"{stem}.tex"
        # \input uses path relative to the .tex file that \inputs the index
...
def main() -> None:
    ...
    for css_path in css_files:
        ...
        md_out_path = os.path.join(MD_OUTPUT_DIR, f"{stem}.md")
        ...
        tex_out_path = os.path.join(TEX_OUTPUT_DIR, f"{stem}.tex")
    ...
    md_index_path = os.path.join(MD_OUTPUT_DIR, "index.md")
    Path(md_index_path).write_text(
        generate_md_index(processed, MD_OUTPUT_DIR), encoding="utf-8"
    )
    print(f"  ✓ MD  index → {md_index_path}")

    tex_index_path = os.path.join(TEX_OUTPUT_DIR, "index.tex")
    Path(tex_index_path).write_text(
        generate_tex_index(processed, TEX_OUTPUT_DIR), encoding="utf-8"
    )
