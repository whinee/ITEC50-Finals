...
DISCLAIMER: Generated by Claude, duly modified.
...
def resolve_files() -> list[str]:  # noqa: C901
    ...
    exclude_folder_paths = [(ROOT_FOLDER / f).resolve() for f in EXCLUDE.get("folders", [])]
...
def parse_css(path: str) -> tuple[list[CSSNode], list[LintError]]:  # noqa: C901
    ...
    def find_or_create_child(
        parent_list: list[CSSNode], name: str, full_path: str, depth: int,
    ) -> CSSNode:
...
def generate_md(
    css_path: str, roots: list[CSSNode], lint_errors: list[LintError],
) -> str:
    lines: list[str] = [
