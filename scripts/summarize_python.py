r"""Python Documentation Summarizer.
...
def generate_tex_for_module(
    ...
    r"""Produce a complete LaTeX fragment for a single Python module.
...
def generate_tex_index(tex_files: list[Path]) -> str:
    r"""Generate ``index.tex`` manifest of ``\input{}`` calls for module docstrings.
...
def generate_tex_code_index(py_files: list[Path]) -> str:
    r"""Generate ``index-code.tex`` manifest that inlines each Python source file.
    ...
    for py_path in py_files:
        ...
        rel_from_paper = py_path.absolute().relative_to(TEX_MAIN_PAPER_DIR, walk_up=True)
...
def main() -> None:  # noqa: C901
    ...
    code_index_path.write_text(
        generate_tex_code_index(processed_py), encoding="utf-8"
    )
