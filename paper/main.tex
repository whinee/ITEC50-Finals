...
\textbf{DeciMark} is a browser-based \textit{Bookmark Link Manager}---a client-side web application that enables users to save, organize, retrieve, and annotate URLs. According to \citet{abrams1998web}, it belongs to the broader category of \textit{Personal Information Management} (PIM) systems, specifically the subcategory of social and personal bookmarking tools .
...
DeciMark does not attempt to replicate the full feature surface of any single existing tool. Instead, it occupies a deliberate gap: a \textit{zero-dependency, visually structured, schema-enforced} bookmark manager that any user can open in a browser without an account, a server, or a subscription.
