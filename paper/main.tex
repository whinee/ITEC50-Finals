...
% This chapter documents Phase 2 of the rubric:
% CSS Layout Strategy, The Box Model, and Specificity & Inheritance.

\section{Layout Strategy}

% Document your overall approach to page layout here.
% Explain the rationale for the two-tier system (Grid for macro layout,
% Flexbox for component-level arrangements).

\subsection{Grid Layout}

% Document your CSS Grid implementation.
% Include:
%   - Which container(s) use display: grid
%   - The grid-template-columns value (e.g. repeat(auto-fill, minmax(..., 1fr)))
%   - How the grid collapses for list-view toggle
%   - Any named grid areas, if used
%
% Paste a representative CSS snippet using \begin{minted}{css}...\end{minted}.

\subsection{Flexbox Components}

% Document all major uses of Flexbox in the application.
% Suggested coverage:
%   - Navigation bar (justify-content, align-items values)
%   - Individual bookmark cards (flex-direction, gap)
%   - Tag list within cards (flex-wrap, gap)
%   - Toolbar / filter bar layout
%
% Paste representative snippets for at least two components.

\section{The Box Model}

% Document padding/margin decisions and overflow handling.
% Suggested coverage:
%   - Page container centering and lateral padding
%   - Card internal spacing (padding) vs. inter-card spacing (gap or margin)
%   - How long URLs are handled (overflow-wrap, text-overflow, white-space)
%   - Border-radius + overflow: hidden for card clipping
%
% Include a brief explanation of why each decision was made, not just what it is.

\section{Specificity \& Inheritance}

% Provide at least one concrete example of a Contextual Selector or ID Selector
% used to resolve a CSS specificity conflict.
%
% Suggested candidates from the codebase:
%   - The CSS mask-image SVG icon theming technique
%     (base .icon rule vs. .bookmark-card .icon override)
%   - Theme variable scoping (:root vs. [data-theme="dark"])
%   - Any component-scoped override that required a contextual selector
%     to win over a more general rule
%
% Format: paste the conflicting rules, then explain which wins and why.
...
% Document all SVG assets in the project.
% Suggested coverage:
%   - Application logo (inline SVG or external file; how it inherits theme color)
%   - FontAwesome icons via CSS mask-image technique
%     (reference the AI Disclosure chapter for context)
%   - Any other SVG illustrations or UI elements
%
% Explain why SVG was chosen over raster formats for these assets.

\section{Image Optimization Strategy}

% Document how raster images are handled.
% Suggested coverage:
%   - File format choices (WebP primary, JPEG/PNG fallback)
%   - Use of the <picture> element for format negotiation
%   - lazy loading (loading="lazy" attribute)
%   - Any use of Squoosh or equivalent tooling for compression
%   - File size before/after optimization, if recorded
%
% If no raster images are used in the current MVP, state this explicitly
% and explain the design decision.

\section{Viewport Configuration \& Responsiveness}

% Document the responsive design implementation.
% Suggested coverage:
%   - The viewport meta tag in the base Jinja2 template
%   - Media query breakpoints used and what they control
%   - Mobile-first vs. desktop-first approach
%   - Lighthouse performance score (if available)
%
% Note: the fluid Grid layout documented in Chapter~7 may handle
% most responsiveness without breakpoints --- explain the interaction.
...
% Document how bookmark data is stored and managed on the client side.
% Suggested coverage:
%   - The module-level bookmarks array (Array of Objects)
%   - The shape of a bookmark object (fields, types)
%   - Any Sets or Maps used for filter state or deduplication
%   - How data flows from the fetch response into the local array

\section{Conditional Logic \& Loops}

% Document the logic that drives bookmark filtering and rendering.
% Suggested coverage:
%   - The filter pipeline (Array.filter with activeFilters check)
%   - The render loop (for...of or .forEach over filtered results)
%   - Conditional handling of null/missing fields in buildCard()
%   - The view-toggle conditional (card grid vs. list layout)
%
% Paste the relevant JavaScript snippets using \begin{minted}{javascript}...\end{minted}.
% Explain the logic, not just the code.

\section{DOM Output \& User Interaction}

% Document how the application communicates results to the user.
% Suggested coverage:
%   - How bookmark cards are created and inserted (createElement / innerHTML)
%   - Use of console.log() for debugging (and whether it is stripped in production)
%   - Event listeners attached to interactive elements (view toggle, tag filter, CRUD buttons)
%   - Fetch API calls for CRUD operations and how responses update the DOM
%   - Error state display (e.g., error banners on failed fetches)

\section{The Client--Server Model in Practice}

% Reflect on how the client-server model shaped implementation decisions.
% This section directly addresses the rubric reflection prompt:
% ``How did understanding the Client-Server model change your perspective
% on what happens when a user clicks a button on your site?''
%
% Suggested coverage:
%   - The distinction between a DOM event and a confirmed server write
%   - Why the UI updates only after a successful response code
%   - How the FastAPI backend acts as the source of truth
%   - Any optimistic UI patterns used (or deliberately avoided) and why
%   - The most challenging Cascade conflict encountered during development
