...
\textbf{BookmarkJD} is a browser-based \textit{Bookmark Link Manager}---a client-side web application that enables users to save, organize, retrieve, and annotate URLs. According to \citet{abrams1998web}, it belongs to the broader category of \textit{Personal Information Management} (PIM) systems, specifically the subcategory of social and personal bookmarking tools .
 
The application requires no server, no account, and no installation. It runs entirely as a static \texttt{HTML/CSS/JS} artifact in any modern browser, making it immediately accessible to students and professionals alike.
...
Most critically: as of late 2024, Pinboard entered a period of significant operational decline. The service is maintained by a single developer who became largely unresponsive to support inquiries, with users reporting increasing downtime, throttled API access (limiting downloads to 100 pins at a time despite users holding tens of thousands of pins), and no public roadmap \citep{tsai2024pinboard}. Long-time subscribers publicly migrated to alternatives.
...
This is a meaningful lesson in the fragility of single-maintainer SaaS dependencies---and a direct motivation for our \textit{offline-first, zero-dependency} architecture.
...
\begin{itemize}
    ...
    \item \textbf{Cloud-only, no offline mode.} Raindrop.io has no local-first or offline-capable mode; all data is held on their servers.
...
\hline
\textbf{Feature} & \textbf{Pinboard} & \textbf{linkding} & \textbf{Raindrop.io} & \textbf{BookmarkJD (Ours)} \\
\hline
...
\hline
Free \& accessible     & Paid only         & Self-host (Docker)& Freemium             & Static HTML (no server) \\
\hline
Service reliability    & Declining (2024)  & Stable            & Stable               & Offline-first \\
\hline
Mobile responsive      & Poor              & Basic             & Good                 & Mobile-first \\
\hline
Setup complexity       & Low               & High (Docker)     & Low                  & Zero (open file) \\
\hline
...
% ── ── ── ── ── ── ── ── ── ── ── ── ── ── ── ── ── ── ── ──
\section{Our Project: BookmarkJD}
 
\subsection{Design Philosophy \& Differentiation}
 
BookmarkJD does not attempt to replicate the full feature surface of any single existing tool. Instead, it occupies a deliberate gap: a \textit{zero-dependency, visually structured, schema-enforced} bookmark manager that any user can open in a browser without an account, a server, or a subscription.
 
The three founding design principles are:
 
\begin{enumerate}
    \item \textbf{Structural discipline over organizational freedom.} Free-form tags and arbitrary folder names produce entropy over time. BookmarkJD enforces the Johnny.Decimal classification scheme as a first-class field on every bookmark, ensuring consistent addressability across users and sessions.
    \item \textbf{Zero barrier to entry.} The application is a static \texttt{index.html} file. It runs offline, requires no backend, and degrades gracefully across browsers.
...
The primary structural innovation in BookmarkJD is the integration of the \textbf{Johnny.Decimal} (JD) organizational system, developed by Johnny Noble (\url{https://johnnydecimal.com}) \citep{johnnydecimal2021}.
...
No existing bookmark manager on the market integrates this system as a native field. This is the primary differentiating feature of BookmarkJD.
...
The following features present in existing tools were intentionally omitted from the MVP scope:
 
\begin{itemize}
    \item \textbf{Cloud synchronization / user accounts}---adds backend complexity that is out of scope for a client-side \texttt{HTML/CSS/JS} implementation.
    \item \textbf{Browser extension}---extension APIs are browser-specific and platform-dependent, placing them outside the course's technology constraints.
    \item \textbf{Full-text page archiving}---requires a server-side crawler; not feasible in a purely static implementation.
\end{itemize}
